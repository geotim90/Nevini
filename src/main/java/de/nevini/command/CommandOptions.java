...
public class CommandOptions {
    ...
    public static CommandOptions parseArgument(String argument) {
        if (StringUtils.isEmpty(argument)) {
            ...
            return new CommandOptions(Arrays.asList(argument.split("\\s+(?=" + CommandPatterns.OPTION + "|" + CommandPatterns.MENTION + ")")));
        }
    }
    ...
    private String supplyArgument() {
        ...
        if (argument.isPresent() && (argument.get().matches(CommandPatterns.OPTION) || argument.get().matches(CommandPatterns.MENTION))) {
            return null;
        } else {
